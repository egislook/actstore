...
export default ({ act, route, store }) => ({
  ...
  APP_INIT: () => {
    ...
    if(token)
      return route.set('index', !route.get('login')).then(() => 
        act(['OPEN', 'USER_FETCH' ])
          .then( ([ socket, user ]) => store.set({ socket, user }) )
      )
...
const actions = ({ act, store }) => ({
  ...
  STATS_FETCH: () => !store.get('stats') && act('GQL', {
    query: `
  ...
  .then(stats => {
    const data = Object.keys(stats)
      .reduce((obj, key) => ({ ...obj, [key]: stats[key].aggregate.count }), {});
